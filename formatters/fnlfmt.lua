...
package.preload["fnlfmt"] = package.preload["fnlfmt"] or function(...)
  ...
  raw_mt = {__fennelview = _1_, __tostring = _2_}
  local function string_3f(ast)
    return ((raw_mt == getmetatable(ast)) and ("string" == type(ast.value)))
  end
  local function number_3f(ast)
    return ((raw_mt == getmetatable(ast)) and ("number" == type(ast.value)))
  end
  local function line(ast)
    if (raw_mt == getmetatable(ast)) then
      return ast.line
    else
  ...
  local function break_pair_3f(let_3f, count, viewed, next_ast, next_next, indent)
    if let_3f then
      ...
      return ((string_3f(next_ast) or tostring(next_ast):find("^&")) and (80 < (indent + 1 + last_line_length(viewed) + 1 + #tostring(next_ast) + #fennel.view(next_next))))
    end
  end
  ...
  if not or_16_ then
    local function _17_(_241)
      local n = 0
      for _ in string.gmatch(_241, "[%z\1-\127\192-\247]") do
        n = (n + 1)
      end
      return n
    end
  ...
  local function view_init_body(t, view, inspector, start_indent, out, callee)
    ...
    if force_initial_newline[callee] then
      ...
      indent = (start_indent + slength(callee))
    end
    local second
    local _21_
    do
      ...
      if (nil ~= t_20_) then
        t_20_ = t_20_["binding-form?"]
      else
      end
      _21_ = t_20_
    end
    if (_21_ and ("unquote" ~= tostring(t[2][1]))) then
      local function _24_()
      ...
      second = view_binding(t[2], view, inspector, (indent + 1), ("let" == callee), _24_())
    else
  ...
  local function space_out_fns_3f(prev, viewed, start_index, i)
    local fn_pattern = "^ *%(fn [^%[]"
    local lambda_pattern = "^ *%(\206\187 [^%[]"
    return (not (start_index == i) and (prev:match(fn_pattern) or prev:match(lambda_pattern) or viewed:match(fn_pattern) or viewed:match(lambda_pattern)))
  end
  local function originally_same_lines_3f(t, n1, n2)
    local first = t[n1]
    local second = t[n2]
    local and_28_ = ("table" == type(second))
    if and_28_ then
      local _29_ = line(first)
      and_28_ = ((line(t) == _29_) and (_29_ == line(second)))
    end
    return and_28_
  end
  local function scalar_3f(form)
    return (("table" ~= type(form)) or fennel["sym?"](form) or fennel["comment?"](form) or fennel["varg?"](form) or string_3f(form) or number_3f(form))
  end
  ...
  local function preserve_same_line_3f(t, i, indent, out, viewed, depth0)
    return (((indent + #table.concat(out) + #viewed) <= 80) and (depth0 <= 3) and not fennel["comment?"](t[(i - 1)]) and (originally_same_lines_3f(t, 1, i) and not viewed:find("\n")))
  end
  ...
  local function pairwise_if_3f(t, indent, i, view)
    ...
    elseif (80 < (indent + 1 + #if_pair(view, select(i, unpack(t))))) then
      ...
      return pairwise_if_3f(t, indent, _38_, view)
    end
  end
  ...
  local renames = {["#"] = "length", ["~="] = "not=", lambda = "\206\187"}
  local function body_form_3f(callee)
    ...
    do
      ...
      if (nil ~= t_43_) then
        t_43_ = t_43_["body-form?"]
      else
      end
      _44_ = t_43_
    end
    return (_44_ or callee:find("%.with-") or callee:find("^with-") or callee:find("%.def") or callee:find("^def"))
  end
  ...
  local function view_pair(t, view, inspector, indent, mt, key)
    ...
    return (maybe_attach_comment(k, indent, _55_()) .. " " .. maybe_attach_comment(v, indent, _60_()))
  end
  ...
  local function view_kv(t, view, inspector, indent)
    ...
    do
      ...
      for _, k in ipairs(keys) do
        local val_28_ = view_pair(t, view, inspector, indent0, mt, k)
        if (nil ~= val_28_) then
          i_27_ = (i_27_ + 1)
          tbl_26_[i_27_] = val_28_
        else
    ...
    local or_70_ = oneline:match("\n")
    if not or_70_ then
      local t_71_ = mt
      if (nil ~= t_71_) then
        t_71_ = t_71_.comments
      else
      end
      if (nil ~= t_71_) then
        t_71_ = t_71_.last
    ...
    if (or_70_ or ((indent0 + #oneline) > inspector["line-length"])) then
      local function _76_()
        local t_75_ = mt
        if (nil ~= t_75_) then
  ...
  local function set_fennelview_metamethod(_idx, form)
    if not scalar_3f(form) then
      if (not fennel["list?"](form) and not fennel["sequence?"](form)) then
        local case_81_ = getmetatable(form)
  ...
  local function parse_form(value, source, raw)
    if (raw and (("string" == type(value)) or ("number" == type(value)))) then
      local _86_
      do
        source["raw"] = raw
        source["value"] = value
      ...
      return setmetatable(_86_, raw_mt)
    else
      return value
    end
  end
  local function format_file(filename, _88_)
    local no_comments = _88_["no-comments"]
    local f
    if (filename == "-") then
      f = io.stdin
    else
    ...
    local parser = fennel.parser(fennel.stringStream(original), filename, {comments = not no_comments, plugins = {{["parse-form"] = parse_form, versions = {"1.5.1"}}}})
    local out = {}
    f:close()
  ...
  return {fnlfmt = fnlfmt, ["format-file"] = format_file, version = "0.3.3-dev"}
end
local _local_92_ = require("fnlfmt")
local format_file = _local_92_["format-file"]
local version = _local_92_.version
local function help()
